...
encGame g =
  unlines $
  [encState g, encInputs g, "guarantee {", encLoc (initial g) ++ ";"] ++
  map
    (\(l, t) -> "G (" ++ encLoc l ++ " -> " ++ encTrans g t ++ ");")
    (Map.toList (trans g)) ++
  ["}"]

encCond :: String -> Set Loc -> String
encCond op loc =
  let locs = Set.toList loc
   in "guarantee { " ++
      op ++ "(" ++ concatMap (\l -> encLoc l ++ " || ") locs ++ "false);}"
...
convert g wc =
  unlines
    [ encGame g
    , case wc of
        Reachability reach -> encCond "F" reach
        Safety safe -> encCond "G" safe
        Buechi fset -> encCond "G F" fset
        CoBuechi fset -> encCond "F G" fset
        _ -> error "Not supported"
    ]
