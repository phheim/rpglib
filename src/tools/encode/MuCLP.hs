...
encPred :: String -> (Symbol -> String) -> [Symbol] -> Loc -> String
encPred name sToStr syms (Loc l) =
  name ++ show l ++ concatMap (\v -> " (" ++ sToStr v ++ ")") syms
...
encTrans pname g =
  \case
    ...
    TSys upds ->
      encOp
        (\(u, l) ->
           encPred pname (\s -> maybe s (encTerm True) (u !? s)) (outputs g) l ++
...
encReach g reach l =
  let head =
   ...
   in head ++
      (if l `elem` reach
         then "true;"
         else encPred "APred" id (outputs g) l ++
...
encSafe g safe l =
  let head =
   ...
   in head ++
      (if l `elem` safe
         then encPred "APred" id (outputs g) l ++
              " /\\ " ++ encFullTrans "APred" g l
         else "false;")
  ...
  let headGFP =
      ...
      headLFP =
        encPred
          ...
          l ++
        ": bool =mu "
   in ( headGFP ++ encPred "LPred" id (outputs g) l ++ ";"
      , headLFP ++
        if l `elem` fset
...
encAll pname g =
  ...
  concatMap
    (\s -> "(" ++ s ++ ": " ++ encSort (ioType g ! s) ++ ")")
    (outputs g) ++
  ". " ++ encPred pname id (outputs g) (initial g)
